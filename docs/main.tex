...
Overall, the usability tests confirmed that the system is accessible and user-friendly, even for users with limited technical background. The results indicate that the system’s design supports smooth interaction by providing clear feedback, reliable operation, and effective handling of user actions.




\section{Results}
The usability tests provided valuable insights into the system's overall performance and user experience. During the initial interaction test, the majority of participants were able to connect to the system and retrieve sensor data without guidance. This indicates that the interface is intuitive and that the primary functions can be accessed with minimal learning. While a few minor delays were observed at the start, these were primarily due to users familiarizing themselves with the interface rather than any usability flaws.

In the second test, which focused on task efficiency and clarity of responses, users consistently reported that the system responded quickly and that the information displayed was easy to interpret. Participants expressed confidence in understanding the sensor data, demonstrating that the backend outputs were well-structured and presented in a comprehensible manner.

The final test assessed the system’s handling of errors and feedback. When users performed incorrect actions—such as requesting unavailable resources or disconnecting during data transmission—the system provided clear, informative messages that guided users toward resolution. This indicates that the system is robust in handling unexpected interactions and provides helpful guidance to users.

Overall, the results from these usability tests suggest that the system is user-friendly and accessible, even for individuals with limited technical experience. The design supports smooth interaction, clear communication, and reliable operation, confirming the effectiveness of the system in real-world use scenarios.
